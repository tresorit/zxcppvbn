...
def to_hpp(lst, lst_name):
#   VS2012 can't handle initializer lists of this size
#   return 'extern const char** %s;\n' % (len(lst), lst_name)
    return 'extern const char* {0}[];\nextern const size_t {0}_count;\n\n'.format(lst_name)

def to_cpp(lst, lst_name):
    l = json.dumps(lst)
#   VS2012 can't handle initializer lists of this size
#   return 'const std::array<const std::string, %d> %s = {%s};\n\n' % (len(lst), lst_name, l[1:len(l)-1])
    return 'const char* {0}[{1}] = {{{2}}};\nconst size_t {0}_count = {1};\n\n'.format(lst_name, len(lst), l[1:len(l)-1])
...
def main():
    ...
    with open('../frequency_lists.hpp', 'w') as f:
        f.write('#ifndef FREQUENCY_LISTS_HPP\n')
        f.write('#define FREQUENCY_LISTS_HPP\n\n')
        for lst_name in lst_names:
            f.write(to_hpp(lsts[lst_name], lst_name))
        f.write('\n#endif\n')
    
    with open('../frequency_lists.cpp', 'w') as f: # words are all ascii at this point
        f.write('#include "frequency_lists.hpp"\n\n')
        for lst_name in lst_names:
            f.write(to_cpp(lsts[lst_name], lst_name))
